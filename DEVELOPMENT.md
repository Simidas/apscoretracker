...
| 层级 | 技术 | 版本 | 说明 |
|---|---|---|---|
| 框架 | Next.js | 15.5 | App Router，服务端渲染 |
...
| 部署 | Cloudflare Workers | — | 通过 @opennextjs/cloudflare 适配器原生部署 |
...
├── .open-next/                 # OpenNext 构建输出（gitignore）
│   ├── assets/                 # 静态资源
│   ├── worker.js               # 生成的 Workers 入口
│   └── ...
├── docs/                       # 产品文档
...
├── next.config.mjs             # Next.js 配置（含 OpenNext dev 初始化）
├── wrangler.jsonc              # Cloudflare Workers 配置（JSONC 格式）
...
GitHub Actions (deploy.yml)
      ↓
npm ci → npx opennextjs-cloudflare build → wrangler deploy
      ↓
Cloudflare Workers (apscoretracker)
      ↓
静态资源: .open-next/assets (Workers Assets binding)
...
```jsonc
{
  ...
  "services": [
    {
      // Self-reference service binding，用于缓存
      "binding": "WORKER_SELF_REFERENCE",
      "service": "apscoretracker"
    }
  ],
  "images": {
    // 图片优化服务
...
export default defineCloudflareConfig({
  // 可启用 R2 增量缓存，详见 https://opennext.js.org/cloudflare/caching
});
```
...
- 注册商：Dynadot
- DNS：Cloudflare
- 主域名：apscoretracker.com（已绑定 Workers）
- Workers 子域名：apscoretracker.simidas2017.workers.dev
...
```bash
npm install
npm run dev          # localhost:3000（含 OpenNext dev 初始化）
```
...
| 2026-05-23 | 迁移到 @opennextjs/cloudflare | 替代 next-on-pages，原生 Workers 支持，解决 ASSETS 兼容问题 |
...
- [ ] 图片优化（目前 unoptimized: true，可启用 Cloudflare Images）
- [ ] Lighthouse 性能评分优化（目标 ≥ 90）
- [ ] 启用 R2 增量缓存（open-next.config.ts 中配置）
